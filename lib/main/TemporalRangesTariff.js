...
export default class TemporalRangesTariff {
    ...
    static formatRange(locale, field, value, options) {
        ...
        if (field === ChronoField.MINUTE_OF_DAY && options?.wholeHours) {
            const hourRange = new IntRange(Math.trunc(value.min / 60), Math.trunc(value.max / 60));
     ...
     */
    static parse(locale, monthRange, dayOfMonthRange, dayOfWeekRange, minuteOfDayRange, rates) {
        const p = ChronoFieldFormatter.forLocale(locale);
        return new TemporalRangesTariff(p.parseRange(ChronoField.MONTH_OF_YEAR, monthRange), dayOfMonthRange === "*"
            ? ALL_DAYS_OF_MONTH
            : IntRange.parseRange(splitRange(dayOfMonthRange)), p.parseRange(ChronoField.DAY_OF_WEEK, dayOfWeekRange), p.parseRange(ChronoField.MINUTE_OF_DAY, minuteOfDayRange), rates);
    }
}
