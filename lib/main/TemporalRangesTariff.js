import { ALL_MONTHS, ALL_DAYS_OF_MONTH, ALL_DAYS_OF_WEEK, ALL_MINUTES_OF_DAY, ChronoField, ChronoFieldFormatter, } from "./ChronoFieldFormatter.js";
import { default as IntRange, UNBOUNDED_RANGE, UNBOUNDED_VALUE, } from "./IntRange.js";
...
function clamped(bounds, r) {
    ...
    const min = r.min === null || (bounds.min !== null && r.min < bounds.min)
        ? bounds.min
        : r.min;
    const max = r.max === null || (bounds.max !== null && r.max > bounds.max)
        ? bounds.max
        : r.max;
...
export default class TemporalRangesTariff {
    ...
    constructor(monthRange, dayOfMonthRange, dayOfWeekRange, minuteOfDayRange, rates) {
        this.#monthRange = clamped(ALL_MONTHS, optional(monthRange, "monthRange", IntRange));
        this.#dayOfMonthRange = clamped(ALL_DAYS_OF_MONTH, optional(dayOfMonthRange, "dayOfMonthRange", IntRange));
        this.#dayOfWeekRange = clamped(ALL_DAYS_OF_WEEK, optional(dayOfWeekRange, "dayOfWeekRange", IntRange));
        this.#minuteOfDayRange = clamped(ALL_MINUTES_OF_DAY, optional(minuteOfDayRange, "minuteOfDayRange", IntRange));
    ...
    appliesAt(date, utc) {
        return (date &&
            ...
            this.#rangeApplies(utc
                ...
                : date.getHours() * 60 + date.getMinutes(), this.#minuteOfDayRange, true));
    }
    /**
     ...
     */
    #rangeApplies(value, range, exclusiveEnd) {
        return (!range ||
     ...
     * @throws `TypeError` if `field` is not supported
     */
    static formatRange(locale, field, value, options) {
        let bounds;
     ...
     */
    static parse(locale, monthRange, dayOfMonthRange, dayOfWeekRange, minuteOfDayRange, rates, options) {
        const p = ChronoFieldFormatter.forLocale(locale);
        return new TemporalRangesTariff(p.parseRange(ChronoField.MONTH_OF_YEAR, monthRange, options), IntRange.parseRange(dayOfMonthRange, ALL_DAYS_OF_MONTH, options), p.parseRange(ChronoField.DAY_OF_WEEK, dayOfWeekRange, options), p.parseRange(ChronoField.MINUTE_OF_DAY, minuteOfDayRange, options), rates);
    }
}
