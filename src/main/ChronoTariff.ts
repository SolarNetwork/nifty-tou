...
export default class ChronoTariff {
	...
	#timeFraction(from: Date, to: Date, refFrom?: Date, refTo?: Date): number {
		if (!(refFrom && refTo)) {
			return 0;
		}
	...
	#dayQuantity(from: Date, to: Date, utc?: boolean): number {
		let q = 0;
		let ref: Date;
	...
	#weekQuantity(from: Date, to: Date, utc?: boolean): number {
		let q = 0;
	...
	#monthQuantity(from: Date, to: Date, utc?: boolean): number {
		let q = 0;
		let ref: Date | undefined;
